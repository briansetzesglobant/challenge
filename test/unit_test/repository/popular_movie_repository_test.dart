...
import 'popular_movie_repository_test.mocks.dart';
...
void main() {
  ...
  group('Repository test', () {
    test(
      'getPopularMoviesList() should get the status success when service state is success and database data is not empty',
      () async {
        when(apiService.getPopularMoviesList())
        ...
        when(challengeDataBase.insertMovieEntity(movieEntity))
            .thenAnswer((_) async => true);
        final response = await repository.getPopularMoviesList();
        verify(apiService.getPopularMoviesList()).called(1);
    ...
    test(
      'getPopularMoviesList() should get the status success when service state is empty and database data is not empty',
      () async {
        when(apiService.getPopularMoviesList())
            .thenAnswer((_) async => dataStateEmpty);
        when(challengeDataBase.selectMovieEntityByCategory('Popular'))
            .thenAnswer((_) async => [movieEntity]);
        final response = await repository.getPopularMoviesList();
        verify(apiService.getPopularMoviesList()).called(1);
        verify(challengeDataBase.selectMovieEntityByCategory('Popular'))
            .called(1);
        expect(
          response.data,
          dataStateSuccess.data,
    ...
    test(
      'getPopularMoviesList() should get the status success when service state is failed and database data is not empty',
      () async {
        when(apiService.getPopularMoviesList())
    ...
    test(
      'getPopularMoviesList() should get the status empty when service state is empty and database data is empty',
      () async {
        when(apiService.getPopularMoviesList())
            .thenAnswer((_) async => dataStateEmpty);
        when(challengeDataBase.selectMovieEntityByCategory('Popular'))
    ...
    test(
      'getPopularMoviesList() should get the status empty when service state is failed and database data is empty',
      () async {
        when(apiService.getPopularMoviesList())
            .thenAnswer((_) async => dataStateFailed);
        when(challengeDataBase.selectMovieEntityByCategory('Popular'))
    ...
    test(
      'getPopularMoviesList() should get the status failed when service state is success and database fail',
      () async {
    ...
    test(
      'getPopularMoviesList() should get the status failed when service state is empty and database fail',
      () async {
        Exception exception = Exception();
        when(apiService.getPopularMoviesList())
            .thenAnswer((_) async => dataStateEmpty);
    ...
    test(
      'getPopularMoviesList() should get the status failed when service state is failed and database fail',
      () async {
        Exception exception = Exception();
        when(apiService.getPopularMoviesList())
            .thenAnswer((_) async => dataStateFailed);
